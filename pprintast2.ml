...
(* control if substitution-related attributes are printed *)
let print_subst_attributes = try
  match Sys.getenv "STEPPER_PRINT_SUBST_ATTRIBUTES" with
    "on" | "true" | "yes" -> true
  | _ -> false
...
and expression ctxt f x =
  let x_pexp_attributes =
    List.filter (fun ({txt = t}, _) ->
                   print_subst_attributes || t <> "stepper.subst" &&
                   t <> "stepper.fun" && t <> "stepper.letrec")
                x.pexp_attributes in
  let x = {x with pexp_attributes = x_pexp_attributes} in
  ...
  if x.pexp_attributes <> [] then expression ctxt f x
    ...
    | Pexp_construct _  when is_simple_construct (view_expr x) ->
        (match view_expr x with
         ...
         | `list xs ->
             pp f "@[<hv0>[%a]@]"
               (list (expression (under_semi ctxt)) ~sep:";@;") (truncate xs)
